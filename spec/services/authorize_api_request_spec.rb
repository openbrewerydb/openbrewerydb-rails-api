    ...
    context 'when invalid request' do
      context 'when missing token' do
        it 'raises a MissingToken error' do
          expect { invalid_request.call }.to raise_error ExceptionHandler::MissingToken
        end
      end
      ...
      context 'when invalid token' do
        subject(:invalid_request) do
          invalid_user_id = 100
          described_class.new('Authorization' => token_generator(invalid_user_id))
        end

        it 'raises an InvalidToken error' do
          expect { invalid_request.call }.to raise_error ExceptionHandler::InvalidToken
        end
      end
